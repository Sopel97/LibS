...
namespace ls
{
    ...
    */
    ...
    template <class T>
    std::optional<T> timeToImpact(const Moving<Circle2<T>>& movingCircle, const Moving<Rectangle2<T>>& movingRectangle);

    template <class S1, class S2, class T = typename S1::ValueType>
    std::optional<ContinuousCollision2<T>> continuousCollision(const Moving<S1>& movingShape1, const Moving<S2>& movingShape2);
	...
	// shortest vector of penetration
	template <class T>
	Vec2<T> penetration(const Circle2<T>& c1, const Circle2<T>& c2);

	// separation of shapes
	template <class S1, class S2, class VectorType = typename S1::VectorType, class ValueType = typename VectorType::ValueType>
